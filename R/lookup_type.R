...
lookup_type <- function(model, query, join_by = "|", verbose = FALSE){
	...
	if(grepl("\\[|\\]",  query)){
      ...
      # Only allow specification of var's parents
		 	...
		 	not_parents      <- model$variables[!model$variables %in% v_parents]
		 	not_parents      <- not_parents[not_parents != var]
		 	not_parents_l    <- str_detect(q, not_parents )
		 	not_parents      <- not_parents[not_parents_l]

		 	if(any(not_parents_l)){
		 	conjugation <- ifelse(length(not_parents)>1, "are not parents of", "is not a parent of")
		 	subjects <- paste0(not_parents, collapse = ", ")
