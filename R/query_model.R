...
#'  query_distribution(model,
#'    query = "(Y[X=1] > Y[X=0])", given = "X==1 & Y ==1")
#'
#'  # Linear query conditional on potential outcomes
#'  query_distribution(model,
#'    query = "(Y[X=1] - Y[X=0])", given = "Y[X=1]==0")
#'
...
#'  # For instance, we are quite sure that X causes Y but we do not
...
#'
#' Calculated from a parameter vector, from a prior or from a posterior
#' distribution.
#'
#' Queries can condition on observed or counterfactual quantities.
#' Nested or "complex" counterfactual queries of the
#' form \code{Y[X=1, M[X=0]]} are allowed.
#'
#' @inheritParams CausalQueries_internal_inherit_params
...
#' @param case_level Logical. If TRUE estimates the probability of the query for a case.
#' @param cred_low lower credibility value: defaults to 0.025
#' @param cred_high upper credibility value: defaults to 0.975
#' @param query alias for queries
...
check_args <-
  function(model,
    ...
    if ((fun == "query_distribution") &&
        (!is.null(given)) && (length(given) != length(queries))) {
      stop(
        "You must specify a given for each query. Use '', 'All', 'ALL',
        'all', 'None', 'none', 'NONE' or 'TRUE' to indicate no given."
      )
    }

    if (any(!using %in% c("priors", "posteriors", "parameters"))) {
      stop("`using` may only take values: `priors`, `posteriors`, or
           `parameters`")
    }
...
get_type_distributions <-
  function(jobs, model, n_draws, parameters = NULL) {
    unique_jobs <-
    ...
    for (i in 1:nrow(jobs)) {
      ...
      if (all(!given)) {
        ...
        if (using_i != "parameters") {
          ...
          if (!case_level_i) {
            estimand <-
              (x %*% type_distribution[given, , drop = FALSE]) / apply(type_distribution[given, , drop = FALSE], 2, sum)
          }
          # case level
          if (case_level_i) {
            estimand <-
              mean(x %*% type_distribution[given, , drop = FALSE]) / mean(apply(type_distribution[given, , drop = FALSE], 2, sum))
          }
        }
      }
