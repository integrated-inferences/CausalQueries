#' make_priors2
#'
#' This function creates the priors to be passed on nodal types
#' 
#' Function takes as arguments the following:
#' @param model A mode created with make_model
#' @param parameter_set A node
#' @param statement A query
#' @param confound A confound statement
#' @param label The name of a nodal type
#' @param prior_distribution A common priors distribution (uniform, jeffreys or certainty)
#' @param alphas Hyperparamenters of the Dirichlet distribution
#' 
#' @export
                         ...
                         label=NA)
{
  ...
  if (is.na(prior_distribution) & is.null(alphas))
  {
    ...
    warning("make_priors2 should have specified arguments. setting prior distribution 'uniform' as default.")
