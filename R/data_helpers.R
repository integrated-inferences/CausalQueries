...
#' @param drop_impossible Logical. Drops data that is impossible given model restrictions.
#' @param drop_none Logical. Drops row for no data.
#' @param mapping_only Logical. Returns data mapping matrix only.
...
get_data_families <- function(model, drop_impossible = TRUE, drop_none = TRUE, mapping_only = FALSE){
	...
	full_data <- merge(all_data, reveal_outcomes(model))
	full_data <- dplyr::filter(full_data, !duplicated(full_data))
...
#'
#' @param data A data.frame of nodes that can take three values: 0, 1, and NA
...
collapse_data <- function(data, model, removeNA = TRUE, remove_family = FALSE, summary = FALSE){
	...
	nodes <- model$nodes
	if(any (!(nodes %in% names(data)))) data[nodes[!(nodes %in% names(data))]] <- NA
	data <- data[, nodes]
	...
	if(!all(unique(data_type) %in% data_families$event))
		message(paste0(unique(data_type)[!(unique(data_type) %in% data_families$event)],
...
#' Drop empty families
#'
#' @param data_events A dataframe whose columns are \code{event}, \code{strategy} and \code{count}
...
all_data_types <- function(model) {
	...
	order_list <-
		c(list(rowSums(is.na(df))),
			lapply(rev(nodes), function(node) is.na(df[,node])),
			lapply( rev(nodes), function(node) df[,node]))

	df[do.call(what = order, args = order_list),]
...
get_possible_data <- function(model, collapse = TRUE){
	...
	possible_data <- lapply(nodes, function(node){
		var_parents <- parents[node][[1]]
		var_nodes <- c(var_parents,node)
		out <- revealed_data[,var_nodes]
		duplicates <- duplicated(out)
		if(length(	var_nodes) == 1) {
			out <- matrix(as.double(out[!duplicates]), ncol = length(var_nodes))
		} else {
			out <- out[!duplicates, ]
		}
		colnames(out) <- var_nodes
		out
	})
	names(possible_data) <- nodes
	if(collapse){
		possible_data <- lapply(nodes, function(node){
			mat <- possible_data[[node]]
			apply(mat, 1, function(x) paste0(node, x, collapse = ""))
		})
	}
...
#'# FLAG -- this is a redundant function called to make ambiguities matrix
...
get_max_possible_data <- function(model) {
	...
	nodes <- 	model$nodes
	max_possible_data <- max_possible_data[nodes]
