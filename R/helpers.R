...
#' @param condition A vector of strings specifying the child node, followed by "|" (given) and the values of its parent nodes in \code{model}.
...
interpret_type <- function(model, condition = NULL, position = NULL){
	...
	interpret <- lapply(1:length(position), function(i){
		...
		data.frame(node = names(position)[i],
							 position = position[[i]],
							 display = display,
