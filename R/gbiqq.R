...
#' @param data_type Either "long" (as made by `simulate_data()`) or "compact" (as made by `summarize_data()``). Compact data must have entries for each member of each strategy family to produce a valid simplex.
...
#' data_short  <- summarize_data(model,data_long)
#' fit <- fitted_model()
