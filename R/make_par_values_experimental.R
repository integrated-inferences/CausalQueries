...
make_par_values_exp <- function(model,
                                  alter = "priors",
                                  x = NA,
                                  alter_at = NA,
                                  node = NA,
  ...
  if(!is.na(alter_at)){
    ...
    command <- paste("which(",alter_at,")", sep = "")
  ...
  if(!is.na(param_names)){
    ...
    command <- paste("which(param_names %in% ", param_names, ")", sep = "")
  ...
  if(any(!is.na(list(node, nodal_type, param_set, given, statement)))){
    ...
    command <- paste("which(", paste(sub_mutate_at, collapse = " & "), ")", sep = "")
