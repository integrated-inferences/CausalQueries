...
diagnose_strategy <- function(reference_model,
	...
	if(length(possible_data_args$N)==1){
		...
		diagnosis           <- data.frame(t(as.matrix(c(apply(out, 1, mean), sims = sims), nrow = 1)),
																			stringsAsFactors = FALSE)
	}
  ...
  return_list$replicates          <- out
	return_list$diagnosis <- diagnosis
	class(return_list) <- "strategy_diagnosis"
	return(return_list)
}
...
#' @export
print.strategy_diagnosis <- function(x, ...) {
	print(summary(x))
	invisible(x)
}
...
#' @export
summary.strategy_diagnosis <- function(object, ...) {
	structure(object, class = c("summary.strategy_diagnosis", "data.frame"))

}

#' @export
print.summary.strategy_diagnosis <- function(x, ...){
...
#' @export
print.data_strategy <- function(x, ...) {
	print(summary(x))
	invisible(x)
}
...
#' @export
summary.data_strategy <- function(object, ...) {
	structure(object, class = c("summary.data_strategy", "data.frame"))

}

#' @export
print.summary.data_strategy <- function(x, ...){

	print(x$possible_data)
