...
#'   \item \code{"priors"}  a data frame of the parameter prior distribution,
#'   \item \code{"posteriors"}  a data frame of the parameter posterior distribution,
...
grab <- function(model, object = NULL, ...) {
  switch(
    ...
    priors = get_param_dist(model, using = "priors", ...),
    posteriors = get_param_dist(model, using = "posteriors", ...),
