...
.inspect.grab <- function(model, what = NULL, print = TRUE, ...) {
  ...
  if (!(what %in% c(.small, .large))) {
    stop("The object requested via what argument is not supported.
         ...
         'causal_types', 'nodal_types', 'data_types', 'ambiguities_matrix', 'type_prior',
         'prior_hyperparameters', 'prior_event_probabilities', 'prior_distribution',
         'posterior_distribution', 'posterior_event_probabilities', 'type_distribution',
         'data', 'stanfit',
  ...
  if (is.null(what)) {
    ...
    if (what %in% c("stanfit", "data", "type_distribution", "stan_summary", "stan_warnings")) {
