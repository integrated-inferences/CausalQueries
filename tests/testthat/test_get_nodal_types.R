...
testthat::test_that(
	...
	code = {
		...
		out_false <- CausalQueries:::make_nodal_types(make_model("X -> Y"), include_node_names = FALSE)
...
testthat::test_that(
    ...
    expect_true(all(grepl("X", CausalQueries:::collapse_nodal_types(nodal_types, include_node_names = TRUE)$X)))
