...
We can draw data directly from the model, in which case a parameter vector is a draw from the priors and used to generate data. Alternatively we can imagine a "true" parameter vector and use that to simulate data. Here is an example in which the parameter vector---with entries corresponding to the rows of the parameter matrix --- imply  that $X=1$ with probability .5, that $X$ has a positive effect on $Y$ for half the units, and that $X$ never has a negative effect on $Y$. 
